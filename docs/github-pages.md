...
Set the repository Pages build type to **GitHub Actions** manually. The workflow cannot reliably change this setting with `GITHUB_TOKEN`; GitHub may return `Resource not accessible by integration`. If the repository remains on branch-based Pages, GitHub's `pages-build-deployment` workflow can publish the repository source `index.html`, which leaves the browser stuck on the deployment check and tries to load `/src/main.tsx`.

The deploy workflow verifies the live page after deployment and fails if GitHub Pages is still serving raw source instead of the built artifact.

...
The deploy workflow attempts to enforce the repository Pages build type as **GitHub Actions** by setting `build_type=workflow` before uploading the artifact. GitHub can still reject token-based settings changes for some repositories; if that happens, set it manually in **Settings → Pages → Build and deployment → Source = GitHub Actions**. If the repository remains on branch-based Pages, GitHub's `pages-build-deployment` workflow can publish the repository source `index.html`, which leaves the browser stuck on the deployment check and tries to load `/src/main.tsx`.

The deploy workflow verifies the live page after deployment and fails if GitHub Pages is still serving raw source instead of the built artifact. Keep that failure strict so branch-source misconfiguration is detected immediately instead of being mistaken for a Supabase or Discord OAuth issue.

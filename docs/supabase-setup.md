...
5. In **Supabase → Authentication → URL Configuration**, set the site URL to the deployed Questboard origin and add the app callback route to the redirect allow list, for example `https://code-smithy.github.io/questboard/?auth_callback=1`. For local Vite development, also allow `http://localhost:5173/?auth_callback=1`.
...
If Discord authorization succeeds but Supabase redirects to `http://localhost:3000/?error=server_error...`, update **Supabase → Authentication → URL Configuration**. `http://localhost:3000` is Supabase's default site URL, so it means the deployed Questboard callback is not configured or not allowed. Set the site URL to `https://code-smithy.github.io/questboard` and add `https://code-smithy.github.io/questboard/?auth_callback=1` to the redirect allow list, then try the login flow again.
...
- Calendar feed URLs are bearer secrets. Regenerating or revoking a feed invalidates the previous subscription URL.
